...
export const CreateRemoveDangerousCallsPlugin = (blocked_globals, blocked_properties) => {
    return function () {
        return function (tree) {
            walk(tree, blocked_globals ?? BLOCKED_GLOBALS, blocked_properties ?? BLOCKED_PROPERTIES);
            return tree; // Must return the tree for RSC
        };
    };
};
function walk(node, blocked_globals, blocked_properties) {
    ...
    if (node.type === 'Identifier' && blocked_globals.includes(node.name)) {
        ...
        const isProperty = parent?.type === 'MemberExpression' &&
    ...
    if (node.type === 'CallExpression' &&
        ...
        throw new Error(`Security: Function calls on computed properties of '${node.callee.object.name}' are not allowed`);
    }
    ...
    if (node.type === 'MemberExpression') {
        ...
        if (node.computed &&
            ...
            throw new Error(`Security: Computed property access on '${node.object.name}' is not allowed`);
        }
        // Block access to blocked global objects
        if (node.object?.type === 'Identifier' &&
            blocked_globals.includes(node.object.name)) {
            throw new Error(`Security: Access to '${node.object.name}' properties is not allowed`);
        }
    }
